...
done < <(git diff --numstat --find-renames=100% "$merge_base" "$head" --)
...
printf '%s\n' \
  ...
  "Generated files: $generated_lines" \
...
done < <(git diff --name-only --no-renames --diff-filter=ACMRTUXB "$merge_base" "$head" --)

sensitive_pattern='-----BEGIN ([A-Z0-9]+ )*PRIVATE KEY-----|https?://[^[:space:]/:@]+:[^[:space:]@]+@|/(Users|home)/[A-Za-z0-9._-]+/|[A-Za-z]:\\Users\\[^\\[:space:]]+|gh[pousr]_[A-Za-z0-9]{20,}|github_pat_[A-Za-z0-9_]{20,}|AKIA[A-Z0-9]{16}|sk-[A-Za-z0-9]{20,}'
